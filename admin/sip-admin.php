...
class SIP_Admin {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'wp-color-picker-alpha', SIP_URL . '/admin/js/wp-color-picker-alpha.js', array( 'wp-color-picker' ), SIP_VERSION, true );

		if ( defined( 'WC_VERSION' ) ) {
			wp_register_script( 'select2', WC()->plugin_url() . '/assets/js/select2/select2.full.min.js', array( 'jquery' ), '4.0.3' );
			wp_enqueue_script( 'wc-enhanced-select', WC()->plugin_url() . '/assets/js/admin/wc-enhanced-select.min.js', array( 'jquery', 'selectWoo' ), WC_VERSION );
		...
		wp_enqueue_script( 'sip-script', SIP_URL . '/admin/js/editor.js', array( 'jquery', 'underscore', 'wp-util', 'jquery-ui-core', 'jquery-ui-draggable', 'jquery-ui-tooltip' ), SIP_VERSION );
